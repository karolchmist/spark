...
abstract class Optimizer(catalogManager: CatalogManager)
  ...
  override protected def isPlanIntegral(plan: LogicalPlan): Boolean = {
    ...
    plan
      .find(PlanHelper.specialExpressionsInUnsupportedOperator(_).nonEmpty)
      .isEmpty)
  }

  override protected val excludedOnceBatches: Set[String] =
    Set("PartitionPruning", "Extract Python UDFs")
  ...
  def defaultBatches: Seq[Batch] = {
    ...
    val batches = (Batch("Eliminate Distinct", Once, EliminateDistinct) ::
      ...
      Batch("Pullup Correlated Expressions", Once, PullupCorrelatedPredicates) ::
  ...
  final override def batches: Seq[Batch] = {
    ...
    val excludedRules = excludedRulesConf.filter { ruleName =>
      ...
      if (nonExcludable) {
        logWarning(
          s"Optimization rule '${ruleName}' was not excluded from the optimizer " +
    ...
    if (excludedRules.isEmpty) {
      ...
      defaultBatches.flatMap { batch =>
        val filteredRules = batch.rules.filter { rule =>
          ...
          if (exclude) {
            logInfo(
              s"Optimization rule '${rule.ruleName}' is excluded from the optimizer."
            )
          }
          !exclude
        }
        if (batch.rules == filteredRules) {
          ...
          logInfo(
            s"Optimization batch '${batch.name}' is excluded from the optimizer " +
...
object RemoveRedundantAliases extends Rule[LogicalPlan] {
  ...
  /**
    ...
    */
  private def removeRedundantAlias(e: Expression,
                                   excludeList: AttributeSet): Expression =
    e match {
  ...
  /**
    ...
    */
  private def removeRedundantAliases(plan: LogicalPlan,
                                     excluded: AttributeSet): LogicalPlan = {
    plan match {
      ...
      case Join(left, right, joinType, condition, hint) =>
        ...
        val newRight =
          removeRedundantAliases(right, excluded ++ newLeft.outputSet)
...
object LimitPushDown extends Rule[LogicalPlan] {
  ...
  private def maybePushLocalLimit(limitExp: Expression,
                                  plan: LogicalPlan): LogicalPlan = {
    (limitExp, plan.maxRowsPerPartition) match {
      case (IntegerLiteral(newLimit), Some(childMaxRows))
          if newLimit < childMaxRows =>
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case LocalLimit(exp, u: Union) =>
      LocalLimit(
        exp,
        u.copy(children = u.children.map(maybePushLocalLimit(exp, _)))
      )
  ...
  def apply(plan: LogicalPlan): LogicalPlan =
    removeProjectBeforeFilter(plan transform {
      ...
      case p @ Project(_, g: Generate) if p.references != g.outputSet =>
        ...
        p.copy(
          child =
            g.copy(child = newChild, unrequiredChildIndex = unrequiredIndices)
        )
      ...
      case p @ Project(_, w: Window)
          if !w.windowOutputSet.subsetOf(p.references) =>
        p.copy(
          child = w.copy(
            windowExpressions =
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transformUp {
    case p1 @ Project(_, p2: Project) =>
      if (haveCommonNonDeterministicOutput(p1.projectList, p2.projectList)) {
        ...
        p2.copy(
          projectList = buildCleanedProjectList(p1.projectList, p2.projectList)
        )
      }
    ...
    case Project(
        l1,
        g @ GlobalLimit(_, limit @ LocalLimit(_, p2 @ Project(l2, _)))
        ) if isRenaming(l1, l2) =>
      val newProjectList = buildCleanedProjectList(l1, l2)
      g.copy(child = limit.copy(child = p2.copy(projectList = newProjectList)))
    case Project(l1, limit @ LocalLimit(_, p2 @ Project(l2, _)))
        if isRenaming(l1, l2) =>
      val newProjectList = buildCleanedProjectList(l1, l2)
      limit.copy(child = p2.copy(projectList = newProjectList))
    case Project(l1, r @ Repartition(_, _, p @ Project(l2, _)))
        if isRenaming(l1, l2) =>
      r.copy(
        child = p.copy(projectList = buildCleanedProjectList(l1, p.projectList))
      )
    case Project(l1, s @ Sample(_, _, _, _, p2 @ Project(l2, _)))
        if isRenaming(l1, l2) =>
      s.copy(
        child =
          p2.copy(projectList = buildCleanedProjectList(l1, p2.projectList))
      )
  }
  ...
  private def isRenaming(list1: Seq[NamedExpression],
    ...
    list1.length == list2.length && list1.zip(list2).forall {
      ...
      case (Alias(a: Attribute, _), b)
          if a.metadata == Metadata.empty && a.name == b.name =>
        true
      case _ => false
    }
  }
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transformUp {
    case w1 @ Window(we1, ps1, os1, w2 @ Window(we2, ps2, os2, grandChild))
        if ps1 == ps2 && os1 == os2 && w1.references
          ...
          WindowFunctionType.functionType(we1.head) == WindowFunctionType
            .functionType(we2.head) =>
...
object TransposeWindow extends Rule[LogicalPlan] {
  private def compatibleParititions(ps1: Seq[Expression],
                                    ps2: Seq[Expression]): Boolean = {
    ps1.length < ps2.length && ps2
      .take(ps1.length)
      .permutations
      .exists(ps1.zip(_).forall {
...
object InferFiltersFromConstraints
  ...
  private def inferNewFilter(plan: LogicalPlan,
...
object EliminateSorts extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case Sort(orders, true, child)
        if SortOrder.orderingSatisfies(child.outputOrdering, orders) =>
      child
    case s @ Sort(_, _, child) => s.copy(child = recursiveRemoveSort(child))
    case j @ Join(originLeft, originRight, _, cond, _)
        if cond.forall(_.deterministic) =>
      j.copy(
        left = recursiveRemoveSort(originLeft),
        right = recursiveRemoveSort(originRight)
      )
  ...
  private def isOrderIrrelevantAggs(aggs: Seq[NamedExpression]): Boolean = {
    def isOrderIrrelevantAggFunction(func: AggregateFunction): Boolean =
      func match {
        ...
        case _: Sum | _: Average | _: CentralMomentAgg =>
          !Seq(FloatType, DoubleType).exists(
            _.sameType(func.children.head.dataType)
          )
        case _ => false
      }
...
/**
  ...
  */
object PushPredicateThroughNonJoin
    extends Rule[LogicalPlan]
    with PredicateHelper {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform applyLocally
  ...
  val applyLocally: PartialFunction[LogicalPlan, LogicalPlan] = {
    ...
    case Filter(condition, project @ Project(fields, grandChild))
        if fields.forall(_.deterministic) && canPushThroughCondition(
          grandChild,
          condition
        ) =>
    ...
    case filter @ Filter(condition, watermark: EventTimeWatermark) =>
      ...
      if (pushDown.nonEmpty) {
        ...
        val newWatermark =
          watermark.copy(child = Filter(pushDownPredicate, watermark.child))
        // If there is no more filter to stay up, just eliminate the filter.
        // Otherwise, create "Filter(stayUp) <- watermark <- Filter(pushDownPredicate)".
        if (stayUp.isEmpty) newWatermark
        else Filter(stayUp.reduceLeft(And), newWatermark)
      } else {
        filter
      }
  ...
  private def pushDownPredicate(filter: Filter, grandchild: LogicalPlan)(
    insertFilter: Expression => LogicalPlan
  ): LogicalPlan = {
  ...
  /**
    ...
    */
  private def canPushThroughCondition(plan: LogicalPlan,
...
object PushPredicateThroughJoin extends Rule[LogicalPlan] with PredicateHelper {
  ...
  private def canPushThrough(joinType: JoinType): Boolean = joinType match {
    case _: InnerLike | LeftSemi | RightOuter | LeftOuter | LeftAnti |
        ExistenceJoin(_) =>
      true
    case _ => false
  }
  ...
  val applyLocally: PartialFunction[LogicalPlan, LogicalPlan] = {
    ...
    case f @ Filter(
          filterCondition,
          Join(left, right, joinType, joinCondition, hint)
    ...
    conditions match {
      ...
      case _ =>
        !conditions
          ...
          .exists(
            refs =>
              refs.exists(join.left.outputSet.contains) && refs
                .exists(join.right.outputSet.contains)
          )
    }
  }
    ...
    case q: LogicalPlan =>
      q transformExpressionsDown {
        case we @ WindowExpression(
              ae @ AggregateExpression(af, _, _, _, _),
              _
            ) =>
          af match {
            case Sum(e @ DecimalType.Expression(prec, scale))
              ...
              MakeDecimal(
                we.copy(
                  windowFunction =
                    ae.copy(aggregateFunction = Sum(UnscaledValue(e)))
                ),
                prec + 10,
                scale
              )
            ...
            case Average(e @ DecimalType.Expression(prec, scale))
              ...
              val newAggExpr =
                we.copy(
                  windowFunction =
                    ae.copy(aggregateFunction = Average(UnscaledValue(e)))
                )
              Cast(
                Divide(
                  newAggExpr,
...
object ConvertToLocalRelation extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case Filter(condition, LocalRelation(output, data, isStreaming))
      ...
      LocalRelation(
        output,
        data.filter(row => predicate.eval(row)),
        isStreaming
      )
  }
  ...
  private def hasUnevaluableExpr(expr: Expression): Boolean = {
    expr
      .find(
        e => e.isInstanceOf[Unevaluable] && !e.isInstanceOf[AttributeReference]
      )
      .isDefined
  }
}
...
object ReplaceIntersectWithSemiJoin extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    case Intersect(left, right, false) =>
      assert(left.output.size == right.output.size)
      val joinCond =
        left.output.zip(right.output).map { case (l, r) => EqualNullSafe(l, r) }
      Distinct(
        Join(
          left,
          right,
...
object RewriteExceptAll extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    case Except(left, right, true) =>
      ...
      val modifiedRightPlan =
        Project(Seq(newColumnRight) ++ right.output, right)
      val unionPlan = Union(modifiedLeftPlan, modifiedRightPlan)
      val aggSumCol =
        Alias(
          AggregateExpression(
            Sum(unionPlan.output.head.toAttribute),
            Complete,
            false
          ),
          "sum"
...
object RewriteIntersectAll extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    case Intersect(left, right, true) =>
      ...
      val leftPlanWithAddedVirtualCols =
        Project(Seq(trueVcol1, nullVcol2) ++ left.output, left)
      val rightPlanWithAddedVirtualCols =
        Project(Seq(nullVcol1, trueVcol2) ++ right.output, right)

      val unionPlan =
        Union(leftPlanWithAddedVirtualCols, rightPlanWithAddedVirtualCols)

      // Expressions to compute count and minimum of both the counts.
      val vCol1AggrExpr =
        Alias(
          AggregateExpression(Count(unionPlan.output(0)), Complete, false),
          "vcol1_count"
        )()
      val vCol2AggrExpr =
        Alias(
          AggregateExpression(Count(unionPlan.output(1)), Complete, false),
          "vcol2_count"
        )()
      ...
      val aggregatePlan = Aggregate(
        left.output,
        Seq(vCol1AggrExpr, vCol2AggrExpr) ++ left.output,
        unionPlan
      )
