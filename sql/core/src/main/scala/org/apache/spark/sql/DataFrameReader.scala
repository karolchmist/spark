...
import org.apache.spark.sql.catalyst.csv.{
  CSVHeaderChecker,
  CSVOptions,
  UnivocityParser
}
...
class DataFrameReader private[sql] (sparkSession: SparkSession)
  ...
  def load(paths: String*): DataFrame = {
    ...
    DataSource
      .lookupDataSourceV2(source, sparkSession.sessionState.conf)
      .map { provider =>
    ...
    sparkSession.baseRelationToDataFrame(
      DataSource
        .apply(
          ...
          className = source,
          options = extraOptions.originalMap
        )
        .resolveRelation()
    )
  }
  ...
  def csv(csvDataset: Dataset[String]): DataFrame = {
    ...
    val linesWithoutHeader: RDD[String] = maybeFirstLine
      .map { firstLine =>
        ...
        filteredLines.rdd.mapPartitions(
    ...
    sparkSession.internalCreateDataFrame(
      parsed,
      schema,
      isStreaming = csvDataset.isStreaming
    )
  }
  ...
  private def assertNoSpecifiedSchema(operation: String): Unit = {
    if (userSpecifiedSchema.nonEmpty) {
      throw new AnalysisException(
        s"User specified schema not supported with `$operation`"
      )
    }
  }
