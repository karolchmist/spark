...
final class DataFrameWriter[T] private[sql] (ds: Dataset[T]) {
    ...
    saveMode.toLowerCase(Locale.ROOT) match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"Unknown save mode: $saveMode. Accepted " +
  ...
  def option(key: String, value: String): DataFrameWriter[T] = {
    this.extraOptions.updated(key, value)
    this
  }
  ...
  def bucketBy(numBuckets: Int,
               colName: String,
               colNames: String*): DataFrameWriter[T] = {
        ...
        case createMode =>
          provider match {
            case supportsExtract: SupportsCatalogOptions =>
              ...
              val location = Option(dsOptions.get("path"))
                .map(TableCatalog.PROP_LOCATION -> _)
            ...
            case _: TableProvider =>
              if (getTable.supports(BATCH_WRITE)) {
                throw new AnalysisException(
                  s"TableProvider implementation $source cannot be " +
  ...
  private def saveToV1Source(): Unit = {
    ...
    runCommand(df.sparkSession, "save") {
      ...
      ).planForWriting(mode, df.logicalPlan)
    }
  }
  ...
  def insertInto(tableName: String): Unit = {
    import df.sparkSession.sessionState.analyzer.{
      AsTableIdentifier,
  ...
  private def assertNotBucketed(operation: String): Unit = {
    if (getBucketSpec.isDefined) {
      if (sortColumnNames.isEmpty) {
        throw new AnalysisException(
          s"'$operation' does not support bucketBy right now"
        )
      } else {
        throw new AnalysisException(
          s"'$operation' does not support bucketBy and sortBy right now"
        )
      }
    }
  }
  ...
  private def saveAsTable(catalog: TableCatalog,
                          ident: Identifier,
    ...
    (tableExists, mode) match {
      ...
      case (true, SaveMode.Overwrite) =>
        ...
        EliminateSubqueryAliases(tableRelation) match {
          ...
          case LogicalRelation(dest: BaseRelation, _, _, _)
  ...
  private def partitioningAsV2: Seq[Transform] = {
    ...
    val bucketing =
      getBucketSpec
        .map(spec => CatalogV2Implicits.BucketSpecHelper(spec).asTransform)
        .toSeq
  ...
  /**
    ...
    */
  private def runCommand(session: SparkSession,
                         name: String)(command: LogicalPlan): Unit = {
